...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    if (editingCategory.value) {
      endpoint = `/api/categories/${editingCategory.value.id}`
    ...
    if (err.response && err.response.data && err.response.data.violations) {
      ...
      formErrors.value = ['Erreur lors de la sauvegarde de la catégorie.']
    }
  } finally {
...
const deleteCategory = async (id) => {
  if (confirm('Voulez-vous vraiment supprimer cette catégorie ?\nSi certains produits ont cette catégorie comme catégorie par défaut, ces produits seront supprimés aussi !')) {
    deletingId.value = id
    try {
      await axios.delete(`/api/admin/categories/${id}/delete-with-products`, {
        headers: {
      ...
      <table class="min-w-full bg-white rounded-lg shadow-md overflow-hidden text-black">
        ...
        >
          ...
          <td class="py-2 px-4">
            ...
            ></span>
            <span class="ml-2 text-xs">{{ category.color }}</span>
          </td>
          <td class="py-2 px-4">
            <ul v-if="category.products && category.products.length">
              <li v-for="prod in category.products" :key="prod.id">
                <router-link
                    :to="`/product/${prod.id}`"
          ...
          <td class="py-2 px-4 flex gap-2">
            ...
            >
              <span v-if="deletingId === category.id" class="loader-xs"></span>
              <span v-else>Supprimer</span>
            </button>
          </td>
        </tr>
        <tr v-if="categories.length === 0">
          <td colspan="5" class="text-center py-6 text-gray-400">Aucune catégorie trouvée.</td>
        </tr>
        </tbody>
      </table>
    </div>
      ...
      <div class="bg-white rounded-lg shadow-lg p-8 w-full max-w-lg relative form-modal-scroll text-black">
        ...
        <form @submit="handleSubmit" class="text-black">
          ...
          <div class="flex justify-end gap-2 mt-6">
            <button type="button" @click="cancelEdit" class="px-4 py-2 bg-gray-300 rounded text-white">Annuler</button>
